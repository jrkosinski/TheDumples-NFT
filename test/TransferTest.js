...
describe("TheDumplesNFT: Transferring", function () {		  
	...
	beforeEach(async function () {
        ...
        //mint 3 tokens and assign to owner 
    ...
    describe("Exceptional Cases", function() {
		it("non-owner cannot transfer a token", async function () {
            await expect(nft.transferFrom(addr1.address, addr2.address, 1)).to.be.revertedWith("ERC721: transfer from incorrect owner"); 
		});
        
		it("non-owner cannot approve a token transfer", async function () {
            await expect(nft.connect(addr1).approve(addr2.address, 1)).to.be.revertedWith("ERC721: approve caller is not owner nor approved for all"); 
		});
        
		it("cannot approve self", async function () {
            await expect(nft.approve(owner.address, 1)).to.be.revertedWith("ERC721: approval to current owner"); 
		});
        
        //TODO: cannot transfer nonexistent token (ERC721: operator query for nonexistent token) 
    });
});